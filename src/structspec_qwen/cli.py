...
class PythonSyntaxProposer:
    ...
    def find_rule(
        ...
        elif re.match(r"^\s*for\s+[A-Za-z_][A-Za-z0-9_]*$", line):
            choose(" in", 0.99, "syntax_for_in")
        elif re.match(r"^\s*for\s+[A-Za-z_][A-Za-z0-9_]*\s+in$", line):
            choose(" range", 0.97, "syntax_for_range")
        elif re.match(r"^\s*for\b.*\brange$", line):
            choose("(", 1.00, "syntax_range_paren")
        elif re.search(r"\bwhile\s+True$", line):
            choose(":\n", 1.00, "syntax_while_true")
        elif re.search(r"\bif\s+__name__$", line):
            choose(" ==", 1.00, "syntax_name_eq")
        elif re.search(r"\bif\s+__name__\s+==$", line):
            choose(' "__main__"', 1.00, "syntax_name_main")
        elif re.search(r'\bif\s+__name__\s+==\s+"__main__"$', line):
        ...
        elif re.search(r"\bsuper\(\)$", line):
            choose(".__init__(", 0.98, "syntax_super_init")
        elif re.search(r"\bsuper\(\)\.__init__\($", line):
            choose(")", 0.90, "syntax_super_close")
        elif re.search(r"\bdef\s+__$", line):
            choose("init", 1.00, "syntax_dunder_init")
        elif re.search(r"\bdef\s+__init$", line):
            choose("__(", 1.00, "syntax_dunder_paren")
        elif re.search(r"\bdef\s+__init__\($", line):
            choose("self", 1.00, "syntax_dunder_self")
        elif re.search(r"\breturn\s+len$", line):
        ...
        elif re.match(r"^\s*return\s+(True|False|None)$", line):
            choose("\n", 1.00, "syntax_return_terminal")
        elif stripped in {"break", "continue", "pass"}:
            choose("\n", 1.00, "syntax_statement_end")
        elif re.match(r"^\s*[ijn]\s*\+=$", line):
            choose(" ", 1.00, "syntax_increment_space")
        elif re.match(r"^\s*[ijn]\s*\+=\s*1$", line):
            choose("\n", 1.00, "syntax_increment_end")
        elif re.search(r"\(n\s*-\s*1\)\s*\+\s*f\(n\s*-$", line):
            choose("2", 1.00, "syntax_fib_n_minus_two")
        elif re.search(r"(\[.*-|\brange\(.*-|\bn\s*-)$", line):
            choose("1", 0.93, "syntax_minus_one")
        elif re.match(r"^.*\bif\b.*\bis\s+(not\s+)?None$", line):
            choose(":\n", 0.97, "syntax_none_guard_colon")
        elif re.match(r"^.*\)\s+as\s+[A-Za-z_][A-Za-z0-9_]*$", line):
...
def run_speculative(
    ...
    while len(gen) < target:
        ...
        t0 = now()
        draft, rules = propose_draft(miner, syntax, gen, max_draft, banned=banned_rules)
